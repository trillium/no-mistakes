...
- Reaps orphaned agent process groups left behind by a crashed daemon or setup wizard - both managed agent servers and the agent processes a pipeline step launched, along with everything those agents spawned, such as test runners. A daemon that exits normally reaps these itself; this pass exists for the kill it could not intercept, and it skips everything when another daemon is still running
