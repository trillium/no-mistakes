...
- PR title: agent-generated from the final branch delta with user intent when available, in conventional commit format (`type(scope): description` or `type: description`); user-facing product impact should use `feat` or `fix` so release automation can pick it up; when a scope is used, it should be the primary affected real module/package from the changed paths and kept broad rather than file-level. If drafting fails, the fallback title is the branch's **first** commit subject (normalized to conventional format), because pipeline fix commits are only ever appended after the authored work. The neutral title `chore: update pull request` is used only when no commit subject can be read at all, and the step logs a loud warning naming it so the title can be corrected before the merge - a squash merge lands the PR title on the default branch verbatim.
...
- On CI failure: fetches failed job logs (GitHub via `gh run view --log-failed`, GitLab via `glab ci trace`, Bitbucket Cloud via failed pipeline step logs; Azure DevOps has no first-class build-log command, so the agent fixes from the failing-check list without logs), sends them to the agent with user intent when available, and, if the agent produces changes, commits them and uses force-push safety guards that prevent dropping the submitted work or pipeline fix commits (see `AGENTS.md` Rebase Base & Force-Push Safety section for details)
