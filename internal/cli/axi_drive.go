...
func newAxiRunCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: "Triggers a pipeline run for the current branch and drives it. Without\n" +
			...
			"--agent overrides the configured pipeline agent for this run only, without\n" +
			"editing ~/.no-mistakes/config.yaml or restarting the daemon. It applies only\n" +
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			return trackAxiSurface("axi-run", "/axi/run", telemetry.Fields{
				...
				"has_agent":  strings.TrimSpace(agentOverride) != "",
			}, func() error {
...
func runAxiRun(cmd *cobra.Command, autoYes bool, skipSteps []types.StepName, intent, agentOverride string) error {
	ctx := cmd.Context()
	...
	if runID == "" {
		...
		var err error
		runID, err = triggerRun(ctx, env, branch, headSHA, skipSteps, intent, agentOverride)
		if err != nil {
...
func triggerRun(ctx context.Context, env *axiEnv, branch, headSHA string, skipSteps []types.StepName, intent, agentOverride string) (string, error) {
	pushOptions := formatSkipPushOptions(skipSteps)
	if opt := formatIntentPushOption(intent); opt != "" {
		pushOptions = append(pushOptions, opt)
	}
	if opt := formatAgentPushOption(agentOverride); opt != "" {
		pushOptions = append(pushOptions, opt)
	}
	...
	if err := env.client.Call(ipc.MethodRerun, rerunParams(env.repo.ID, branch, skipSteps, intent, agentOverride), &rr); err != nil {
...
func rerunParams(repoID, branch string, skipSteps []types.StepName, intent, agentOverride string) *ipc.RerunParams {
	return &ipc.RerunParams{RepoID: repoID, Branch: branch, SkipSteps: skipSteps, Intent: intent, Agent: agentOverride}
}
